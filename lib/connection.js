...
Connection.prototype.connect = function(callback) {
  this.once(CXN_STATES.PROMOTED, callback);
...
Connection.prototype._connect = function() {
  var self = this;
  this.connection = tls.connect(us.pick(this, 'host', 'port', 'ca', 'key'), function(err) {
    if (err) {
      self._error(err);
      return;
    }
    ...
    var onData = function(data) {
      if (data.method === 'handshake.post') {
        ...
        if (true) { // if successful
          self.readable.removeListener('data', onData);
          self.writable.write(self._handshakeMessage());
          self._changeState(CXN_STATES.PROMOTED);
        }
      }
    };
